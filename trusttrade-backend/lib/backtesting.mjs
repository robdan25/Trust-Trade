...
/**
 * Momentum strategy: Buy when momentum > 2%, sell when < -1%
 */
function getMomentumSignal(closes) {
  ...
  if (lastMom > 2.0) return 'buy';
  if (lastMom < -1.0) return 'sell';
  ...
  return [
    {
      id: 'momentum',
      name: 'Momentum',
      description: 'Buys when momentum is strong (>2%), sells on weakness (<-1%)'
    },
    {
      id: 'sma-crossover',
      name: 'SMA Crossover',
