  ...
  symbols.forEach(symbol => {
    const intervalId = setInterval(async () => {
      try {
        ...
        // Generate signal with adaptive strategy (Phase 3)
        const signalResult = await analyzeAndSignal(symbol, interval, {
          useAi: false, // Disable AI to speed up analysis
          useAdvancedIndicators: true, // Use multi-indicator analysis
          useAdaptiveStrategy: true // Use adaptive strategy selection
        });
        ...
        // Assess profit opportunity (legacy check)
        ...
        // Auto-trade based on signal strength AND profit opportunity
        const minConfidence = 60; // Minimum 60% confidence required
        const signalStrong = signalResult.confidence !== null && signalResult.confidence >= minConfidence;

        if (autoTrade && signalStrong && profitAnalysis.profitOpportunity && signalResult.signal !== 'hold') {
