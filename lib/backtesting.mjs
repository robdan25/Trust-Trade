...
export async function runBacktest(config) {
  ...
  // Debug: Log first 5 and last 5 close prices
  ...
  console.log(`\n=== BACKTEST DEBUG (${strategy}) ===`);
  console.log(`Total candles processed: ${candles.length}`);
  console.log(`Buy signals detected: ${buySignals}`);
  console.log(`Sell signals detected: ${sellSignals}`);
...
/**
 * Momentum strategy: Buy when momentum turns positive, sell when turns negative
 * (Simplified for realistic backtesting - any directional change triggers action)
 */
function getMomentumSignal(closes) {
  ...
  // Buy when momentum is positive (uptrend)
  if (lastMom > 0) return 'buy';

  // Sell when momentum is negative (downtrend)
  if (lastMom < 0) return 'sell';
  ...
  return [
    {
      id: 'momentum',
      name: 'Momentum',
      description: 'Buys when 14-period momentum turns positive, sells when negative (trend following)'
    },
    {
      id: 'sma-crossover',
      name: 'SMA Crossover',
